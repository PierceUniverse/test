...
# helper function to calculate the regularization on betas
...
def train(model, loss_fn, optimizer, scheduler=None):
    ...
    for x_var, y_var in tk1:
        ...
        scores = model(x_var)
        _, norm_beta = calculate_avg_beta(model)
...
def test(model, loss_fn, optimizer, phase, shceduler=None):
    model.eval()
    counter = 0
...
if args.test_only == False:
    for epoch in range(num_epochs):
        ...
        df_data = np.array([train_losses, train_betas, valid_losses, valid_acc]).T
        df = pd.DataFrame(df_data, columns=['train_losses', 'train_betas', 'valid_losses', 'valid_acc'])
